...
function CopyButton({ text, isDark }: { text: string; isDark: boolean }) {
  ...
  return (
    <button
      ...
      onMouseEnter={e => {
        (e.currentTarget as HTMLElement).style.color = P.blue;
        (e.currentTarget as HTMLElement).style.borderColor = P.blue;
      }}
      onMouseLeave={e => {
...
export default function HireMeModal({ isOpen, onClose, isDark }: Props) {
  ...
  // All contact rows are built from profile config — no hardcoding
  const contacts = [
    {
      icon: Mail,
      label: "Email",
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: isDark ? "rgba(0,0,0,0.85)" : "rgba(80,120,150,0.35)" }}
      />
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-lg transition-all focus:outline-none"
          style={{ color: t.textMut, border: "1px solid " + t.border }}
          onMouseEnter={e => {
            (e.currentTarget as HTMLElement).style.color = t.textPri;
            (e.currentTarget as HTMLElement).style.borderColor = P.midRed;
          }}
          onMouseLeave={e => {
            (e.currentTarget as HTMLElement).style.color = t.textMut;
            (e.currentTarget as HTMLElement).style.borderColor = t.border;
        ...
        <div className="p-6">
          ...
          {/* Contact rows — all from profile config */}
            ...
            {contacts.map(c => {
              ...
              return (
                <div
                  ...
                  onMouseEnter={e => {
                    (e.currentTarget as HTMLElement).style.background = t.cardHov;
                    (e.currentTarget as HTMLElement).style.borderColor = c.color + "50";
                  }}
                  onMouseLeave={e => {
                    (e.currentTarget as HTMLElement).style.background = t.pillBg;
                    (e.currentTarget as HTMLElement).style.borderColor = t.border;
                  }}
                >
                  {c.href
                  ...
                  {c.copyable && (
                    <CopyButton text={c.value.replace(/\s/g, "")} isDark={isDark} />
                  )}
                </div>
              );
            })}
