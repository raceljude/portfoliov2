  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="absolute inset-0 backdrop-blur-sm"
        style={{ background: isDark ? "rgba(0,0,0,0.82)" : "rgba(100,70,60,0.25)" }}/>
      <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl shadow-2xl"
        style={{ background: t.card, border: `1px solid ${t.border}`, animation: "slideUp 0.3s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
        ...
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-lg transition-all focus:outline-none"
          ...
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = t.textPri; (e.currentTarget as HTMLElement).style.borderColor = experience.color; }}
        ...
        <div className="p-6 pt-5">
          ...
          <h2 className="text-2xl font-bold leading-tight"
            style={{ fontFamily: "var(--font-display)", color: t.textPri }}>{experience.role}</h2>
          <p className="font-medium mt-1 text-sm" style={{ color: experience.color }}>{experience.company}</p>
          <div className="flex flex-wrap gap-4 mt-3 text-xs font-mono" style={{ color: t.textMut }}>
          ...
          <p className="mt-5 text-sm leading-relaxed border-l-2 pl-4"
            style={{ color: t.textSec, borderColor: experience.color }}>{experience.summary}</p>
          <div className="mt-6">
            <h3 className="text-xs font-mono uppercase tracking-widest mb-3" style={{ color: t.textMut }}>What I did</h3>
            <ul className="space-y-2.5">
              {experience.highlights.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-sm" style={{ color: t.textSec }}>
