...
export default function ExperienceDetailSection({ experiences, isDark }: Props) {
  ...
  const hlText  = isDark ? "#c8dce8" : "#1a3a50";
  const dotBg   = isDark ? "#0c1018" : "#f0f4f8";

  return (
    <section id="experience-detail" className="mt-14 scroll-mt-20">
      <div className="flex items-center justify-between mb-8" style={{ animation: "fadeUp 0.6s ease 0.1s forwards", opacity: 0 }}>
        <div>
          <p className="text-xs font-mono uppercase tracking-widest mb-1" style={{ color: textMut }}>Deep dive</p>
          <h2 className="text-xl font-bold" style={{ fontFamily: "var(--font-display)", color: textPri }}>Work History</h2>
        </div>
        <span className="text-xs font-mono" style={{ color: isDark ? "#2a4a60" : "#8ab0c8" }}>{experiences.length} roles</span>
      </div>
          ...
          {experiences.map((exp, index) => (
            <div key={exp.id} className="relative sm:pl-10"
              ...
              <div className="absolute left-0 top-1 w-[15px] h-[15px] rounded-full border-2 hidden sm:flex items-center justify-center"
                style={{ background: exp.color, borderColor: dotBg, boxShadow: exp.status === "current" ? `0 0 12px ${exp.color}80` : "none" }}>
                {exp.status === "current" && <span className="w-[5px] h-[5px] rounded-full animate-ping" style={{ background: exp.color }}/>}
              </div>

              <div className="rounded-xl overflow-hidden" style={{ background: card, border: `1px solid ${border}`, transition: "background 0.3s, border-color 0.3s" }}>
                <div className="h-[3px] w-full" style={{ background: `linear-gradient(90deg, ${exp.color}, transparent 70%)` }}/>
                <div className="p-6">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
                    <div>
                      ...
                      <h3 className="text-lg font-bold leading-tight" style={{ fontFamily: "var(--font-display)", color: textPri }}>{exp.role}</h3>
                    ...
                    <div className="flex flex-col gap-1.5 shrink-0 items-start sm:items-end">
                      <span className="flex items-center gap-1.5 text-[11px] font-mono" style={{ color: textMut }}><Calendar size={10}/>{exp.period}</span>
                      <span className="flex items-center gap-1.5 text-[11px] font-mono" style={{ color: textMut }}><MapPin size={10}/>{exp.location}</span>
                    </div>
                  </div>

                  <p className="text-sm leading-relaxed border-l-2 pl-3 mb-5" style={{ color: textSec, borderColor: exp.color }}>{exp.summary}</p>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="text-[10px] font-mono uppercase tracking-widest mb-3" style={{ color: textMut }}>Responsibilities</h4>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.stack.map(tech => (
                          <span key={tech} className="text-[11px] font-mono px-2.5 py-1 rounded-lg"
                            style={{ border: `1px solid ${border}`, color: isDark ? "#8aa8bc" : "#2a4f66", background: pillBg }}>
                            {tech}
                          </span>
                        ))}
                      </div>
